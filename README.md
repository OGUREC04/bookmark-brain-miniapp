...
> **Это frontend-only репозиторий.** Backend (FastAPI), Telegram-бот, worker и
> схемы БД живут в монорепо `bookmark-brain` (соседняя папка `../bookmark-brain`).
> Конвенции и правила работы для агентов — в `CLAUDE.md`.
...
Источник истины — хэндоф-эталон дизайн-системы (`docs/design-system-miniapp/`
в монорепо `bookmark-brain`, есть HTML-холст). Эти компоненты портированы
...
| `titleOf` | заголовок с guard от `null raw_text` (единая точка) |
...
npm run dev        # Vite dev server :3000, проксирует /api → localhost:8000
npm run build      # tsc (строгий) + vite build — прод-сборка
```

`tsc` падает на любой ошибке типов — это gate (зелёный билд = типы сошлись).

Для живого теста в Telegram backend/worker/bot/ngrok поднимаются из монорепо
`../bookmark-brain` (см. его `.claude/STARTUP.md` — docker postgres+redis, backend,
worker, bot, ngrok). ngrok туннелит :3000 независимо от того, из какого репо поднят Vite.
