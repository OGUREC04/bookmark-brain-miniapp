...
**Остаётся (next, см. лог):** ActionSheet close (× vs «Закрыть» — ждёт решения); барабан-поповер
(тикет `tb1`); recurrence «Повторять» (тикет `8r4`, +бэк); **глобальная sentence-case** (sweep всех
экранов). Бэклог: создание пространства (`8g1`); SuggestionCard (`ntn`); facet-чипы (`0u7`);
медиа QuickCreate (`ti0`); контекст ссылок — бэк (`z9q`).
