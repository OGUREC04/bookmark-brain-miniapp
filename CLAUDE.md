...
(бэк-лимит raw=600). Известное ограничение (флаг, не фикс): если в текст регулярного вписать сами
слова расписания («каждый день в 23:00»), бэк-парсер исказит хранимый текст — радикальное решение =
структурный API вместо raw-строки (затронет и бот-команду `/repeat`).
